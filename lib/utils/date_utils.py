...
def is_day_formatted_as_date(day: str) -> bool:
    try:
...
def get_next_weekday_as_date(weekday_number: int) -> str:
...
def parse_date_to_weekday(date: str) -> str:
    return calendar.day_name[datetime.strptime(date, "%Y-%m-%d").weekday()]


def get_day_number(weekday_name: str) -> int | None:
