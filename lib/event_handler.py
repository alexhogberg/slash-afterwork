...
class EventHandler:
    def __init__(
        ...
        self.event_dal = (
            event_dal or EventMongoDAL(team_id) if event_dal is None else event_dal
        )
    ...
    def handle_interactive_event(self, payload):
        ...
        if "view_id" in payload.get("container"):
            self.logger.info(
                "Updating view with ID: {view_id}".format(
    ...
    def update_events_view(self, user_id):
        ...
        if user_id:
            ...
            self.bolt_client.views_publish(user_id=user_id, view=event_view)
    ...
    def create_event(self, command, event):
        ...
        self.bolt_client.views_open(trigger_id=trigger_id, view=build_create_dialog())
    ...
    def delete_event(self, id, author, channel_id):
        ...
        if id is not None:
            ...
            if self.event_dal.delete_event(id, author):
                ...
                self.send_epemeral_message(
                    f"*Gotcha!* Event deleted: {event_details}",
                    author,
                    channel_id,
                )
                return True
    ...
    def suggest_event(self, command, event):
        ...
        if area is None or area == "":
            return self.bolt_client.chat_postEphemeral(
        ...
        for p in places[:5]:
            return_value["blocks"] = (
                return_value["blocks"] + EventPlace(p).format_block()
            )

        if len(return_value["blocks"]) == 0:
            return self.bolt_client.chat_postEphemeral(
                "No suggestions found for {area}".format(area=area)
            )
    ...
    def create_event_from_input(
        self, date, place_id, time, author, channel_id, description=None
    ):
        """
        ...
        try:
            ...
            self.say(text=message.text, blocks=message.blocks, channel=channel_id)
    ...
    def create_event_response(self, payload):
        ...
        suggested_place = next(
            block["block_id"]
            for block in payload["view"]["blocks"]
            if block["block_id"].startswith("suggest_place")
        )
